...
export default function Expense() {
  ...
  const AddExpense = async (amount, description) => {
    ...
    if(description !== '' && amount > 0){
      const response = await axios.post(`${baseURL}/expense/add`, {amount: amount, description: description}, {
        headers: {
          authorization: `token ${token}` 
        }
      })
  ...
  const DeleteExpense = async (id) => {
    ...
    const response = await axios.delete(`${baseURL}/expense/delete/${id}`, {
      headers: {
        authorization: `token ${token}` 
      }
    })
    ...
    const updateExpense = async () => {
      ...
      if(updateDesc !== '' && updateAmount > 0){
        const response = await axios.put(`${baseURL}/expense/update/${updateId}`, {amount: updateAmount, description: updateDesc} ,{
          headers: {
            authorization: `token ${token}` 
          }
        })
